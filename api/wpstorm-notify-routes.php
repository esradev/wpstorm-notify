...
class Wpstorm_Notify_Routes
{
    ...
    {
        $version   = '1';
        $namespace = 'wpstorm-notify/v' . $version;
    ...
    {
        $wpstorm_notify_settings_options = get_option('wpstorm_notify_settings_options');
        if (empty($wpstorm_notify_settings_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_settings_options;
    }
    ...
    public function add_settings_options($data)
    {
        ...
        return update_option('wpstorm_notify_settings_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_login_notify_options = get_option('wpstorm_notify_login_notify_options');
        if (empty($wpstorm_notify_login_notify_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_login_notify_options;
    }
    ...
    public function add_login_notify_options($data)
    {
        ...
        return update_option('wpstorm_notify_login_notify_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_phonebook_options = get_option('wpstorm_notify_phonebook_options');
        if (empty($wpstorm_notify_phonebook_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_phonebook_options;
    }
    ...
    public function add_phonebook_options($data)
    {
        ...
        return update_option('wpstorm_notify_phonebook_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_comments_options = get_option('wpstorm_notify_comments_options');
        if (empty($wpstorm_notify_comments_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_comments_options;
    }
    ...
    public function add_comments_options($data)
    {
        ...
        return update_option('wpstorm_notify_comments_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_newsletter_options = get_option('wpstorm_notify_newsletter_options');
        if (empty($wpstorm_notify_newsletter_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_newsletter_options;
    }
    ...
    public function add_newsletter_options($data)
    {
        ...
        return update_option('wpstorm_notify_newsletter_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_woocommerce_options = get_option('wpstorm_notify_woocommerce_options');
        if (empty($wpstorm_notify_woocommerce_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_woocommerce_options;
    }
    ...
    public function add_woocommerce_options($data)
    {
        ...
        return update_option('wpstorm_notify_woocommerce_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_edd_options = get_option('wpstorm_notify_edd_options');
        if (empty($wpstorm_notify_edd_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_edd_options;
    }
    ...
    public function add_edd_options($data)
    {
        ...
        return update_option('wpstorm_notify_edd_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_aff_options = get_option('wpstorm_notify_aff_options');
        if (empty($wpstorm_notify_aff_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_aff_options;
    }
    ...
    public function add_aff_options($data)
    {
        ...
        return update_option('wpstorm_notify_aff_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_membership_options = get_option('wpstorm_notify_membership_options');
        if (empty($wpstorm_notify_membership_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_membership_options;
    }
    ...
    public function add_membership_options($data)
    {
        ...
        return update_option('wpstorm_notify_membership_options', $option_json);
    }
    ...
    {
        $wpstorm_notify_integrations_options = get_option('wpstorm_notify_integrations_options');
        if (empty($wpstorm_notify_integrations_options)) {
            return new WP_Error('no_option', 'Invalid options', ['status' => 404]);
        }

        return $wpstorm_notify_integrations_options;
    }
    ...
    public function add_integrations_options($data)
    {
        ...
        return update_option('wpstorm_notify_integrations_options', $option_json);
    }
    ...
    {
        if (!Wpstorm_Notify_Base::$woo_phonebook_id) {
            return 'noPhonebook';
        } else {
    ...
    {
        if (!Wpstorm_Notify_Base::$digits_phonebook_id) {
            return 'noPhonebook';
        } else {
            ...
            foreach ($users as $userid) {
                ...
                $list[] = (object) [
                    'number'       => $number,
                    'name'         => $name,
                    'phonebook_id' => (int) Wpstorm_Notify_Base::$digits_phonebook_id
                ];
            }
            Wpstorm_Notify_Ippanel::save_list_of_phones_to_phonebook($list);
    ...
    {
        if (!Wpstorm_Notify_Base::$bookly_phonebook_id) {
            return 'noPhonebook';
        } else {
            global $wpdb;
            ...
            foreach ($bookly_customers as $customer) {
                ...
                $list[] = (object) [
                    'number'       => $number,
                    'name'         => $name,
                    'phonebook_id' => (int) Wpstorm_Notify_Base::$bookly_phonebook_id
                ];
            }
            Wpstorm_Notify_Ippanel::save_list_of_phones_to_phonebook($list);
    ...
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'wpstorm_notify_newsletter';
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wpstorm_notify_newsletter';
    ...
    public static function add_gravity_forms_action_to_db($incomingData)
    {
        ...
        $table_name = $wpdb->prefix . 'wpstorm_notify_gravity_forms';
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wpstorm_notify_gravity_forms';
    ...
    public static function send_sms($data)
    {
        ...
        if ($sender === '1') {
            $sender = Wpstorm_Notify_Base::$fromNum;
        } else {
            $sender = Wpstorm_Notify_Base::$fromNumAdver;
        }
        ...
        if (!str_contains($phones, ',')) {
            if (Wpstorm_Notify_Base::validate_mobile_number($phones)) {
                $fixed_phones[] = Wpstorm_Notify_Base::validate_mobile_number($phones);
            }
        } else {
            $phones = explode(',', $phones);
            foreach ($phones as $phone) {
                if (Wpstorm_Notify_Base::validate_mobile_number($phone)) {
                    $fixed_phones[] = Wpstorm_Notify_Base::validate_mobile_number($phone);
                }
            }
        }
        $sendToPhones_response = Wpstorm_Notify_Ippanel::send_message($fixed_phones, $message, $sender);

        // Send SMS to phonebooks
        foreach ($phonebooks as $phonebook) {
            $phonebooks_ids[] = $phonebook['value'];
        }
        $sendToPhonebooks_response = Wpstorm_Notify_Ippanel::send_sms_to_phonebooks($phonebooks_ids, $message, $sender);


        // Send SMS to subscribers
        if ($send_to_subscribers) {
            $subscribers = Wpstorm_Notify_Base::get_subscribers();
            ...
            if (str_contains($message, '%name%')) {
                foreach ($subscribers as $subscriber) {
                    ...
                    $sendToSubscribers_response[] = Wpstorm_Notify_Ippanel::send_message([$subscriber->phone], $message_fixed, $sender);
                }
            } else {
                $phones = [];
                foreach ($subscribers as $subscriber) {
                ...
                $sendToSubscribers_response[] = Wpstorm_Notify_Ippanel::send_message($phones, $message, $sender);
            }
        }
    ...
    public static function get_lines($auth = [])
    {
        ...
        $body     = [
            'uname' => $auth['username'] ?? Wpstorm_Notify_Base::$username,
            'pass'  => $auth['password'] ?? Wpstorm_Notify_Base::$password,
            'op'    => 'lines',
        ];
    ...
    public static function add_woocommerce_order_action_to_db($incomingData)
    {
        ...
        $table_name = $wpdb->prefix . 'wpstorm_notify_woocommerce_order_actions';
    ...
    {
        global $wpdb;
        $table = $wpdb->prefix . 'wpstorm_notify_woocommerce_order_actions';
...
Wpstorm_Notify_Routes::get_instance();
