(function ($) {
  ...
  otp_verify_button +=
    '<div id="woo_second_buttons"><div class="button alt" id="otp_resend">';
  ...
  $("#billing_phone_send_otp_field").on(
    ...
    function () {
      ...
      otp_send_button.addClass("button--loading").prop("disabled", true);
      let data = {
        action: "send_otp_code",
        mobile: mobile,
      };
      $.post(ajax_url.ajax_url, data, function (response) {
        if (response.success) {
          otp_send_button
          ...
          otp_send_button.removeClass("button--loading");
          $("#response_message")
  ...
  $("#billing_phone_otp").keyup(function () {
    let mobile = $(this).val();
    if (mobile.length < 4) {
      $(this).parents(".otp_field").addClass("woocommerce-invalid");
    }
    if (mobile.length > 4) {
  ...
  $("#billing_phone_otp_button_field").on("click", "#otp_resend", function () {
    ...
    otp_resend_button.addClass("button--loading").prop("disabled", true);
    let data = {
      action: "send_otp_code",
      mobile: mobile,
    };
    $.post(ajax_url.ajax_url, data, function (response) {
      if (response.success) {
        ...
        otp_resend_button.removeClass("button--loading");
        $("#response_message")
          .text("خطایی رخ داد لطفا با مدیریت تماس بگیرید")
          .addClass("error")
          .show();
