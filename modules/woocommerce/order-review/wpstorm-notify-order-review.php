...
/**
 * Class wpstorm-notify_Order_Review.
 */
class Wpstorm_Notify_Order_Review
{
    /**
     * Instance
     *
    ...
    public function __construct()
    {
        ...
        if (self::$woo_poll) {
            ...
            add_shortcode('wpstorm-notify_order_review_landing_page', [$this, 'order_review_page']);
            add_action('init', [$this, 'create_order_review_landing_page'], 99, 0);
            add_action('wp_ajax_wpstorm-notify_submit_order_review', [$this, 'wpstorm-notify_submit_order_review']);
            add_action('wp_ajax_nopriv_wpstorm-notify_submit_order_review', [$this, 'wpstorm-notify_submit_order_review']);
    ...
    public function enqueue_styles()
    {
        ...
        if (is_page('order-review')) {
            ...
            wp_enqueue_style('wpstorm-notify-order-review-css', WPSTORM_NOTIFY_URL . 'assets/css/wpstorm-notify-order-review-page.css', [], WPSTORM_NOTIFY_VERSION, 'all');
        }
    }
    ...
    {
        wp_enqueue_script('wpstorm-notify-order-review-js', WPSTORM_NOTIFY_URL . 'assets/js/wpstorm-notify-order-review-page.js', [], WPSTORM_NOTIFY_VERSION, true);
        wp_localize_script(
            'wpstorm-notify-order-review-js',
            'ajax_object',
    ...
    public function order_review_page($data)
    {
        ...
        ob_start();
        include plugin_dir_path(__FILE__) . 'wpstorm-notify-order-review-page.php';
        $html = ob_get_clean();
    ...
    public function create_order_review_landing_page()
    {
        ...
        if (!$custom_page) {
            ...
            $page = [
                'post_title'    => esc_html__('Order Review', 'wpstorm-notify'),
                'post_name'     => 'order-review',
                'post_status'   => 'publish',
                'post_type'     => 'page',
                'post_content'  => '[wpstorm-notify_order_review_landing_page]'
            ];
            wp_insert_post($page);
        }
    }

    public function wpstorm_notify_submit_order_review()
    {
        // Retrieve form data
        ...
        $data = [
            ...
            'comment_author_email' => $user_email ?? 'info@wpstorm-notify.ir',
    ...
    public function schedule_review_reminder_sms($order_id)
    {
        ...
        if (!empty($phone_number)) {
            ...
            $phone_number = Wpstorm_Notify_Base::normalize_phone_number($phone_number);
    ...
    {
        $woo_poll         = self::$woo_poll;
        $woo_poll_time    = self::$woo_poll_time;
        $woo_poll_message = self::$woo_poll_msg;
        if ($woo_poll !== true || empty($woo_poll_time) || empty($woo_poll_message)) {
            return;
        }
        ...
        $date_to_send = date('Y-m-d H:i:s', strtotime($order->get_date_created()->date('Y-m-d H:i:s') . ' + ' . $woo_poll_time . ' days'));
        ...
        Wpstorm_Notify_Ippanel::send_timed_sms($phone_number, $time_tehran, $message);
    }
}

Wpstorm_Notify_Order_Review::get_instance();
