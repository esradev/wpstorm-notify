...
class Wpstorm_Notify_Edd
{
    ...
    {
        $edd_options = json_decode(get_option('wpstorm-notify_edd_options'), true);
        if ($edd_options) {
        ...
        add_action('edd_purchase_form_user_info_fields', [$this, 'show_mobile_field_checkout_field']);
        add_action('edd_built_order', [$this, 'store_mobile_meta'], 10, 1);
        add_action('edd_checkout_error_checks', [$this, 'validate_mobile_field'], 10, 2);
        add_action('edd_complete_purchase', [$this, 'edd_complete_purchase_action'], 10, 3);
    }
    ...
    public function edd_view_order_details($order_id)
    {
        ...
        <div class="column-container">
            ...
            <div class="column">
                <strong><?php esc_html_e('Phone number: ', 'wpstorm-notify'); ?></strong>
     ...
     * Store mobile meta.
     */
    public function store_mobile_meta($order_id)
    {
     ...
     */
    public function edd_complete_purchase_action($payment_id)
    {
        ...
        if (self::$edd_send_to_admin) {
            $this->send_edd_sms(Wpstorm_Notify_Base::$admin_number, self::$edd_admin_pattern, $data);
        }
     ...
     */
    public function show_mobile_field_checkout_field()
    {
        ?>
        <p id="edd-phone-wrap">
            <label class="edd-label" for="edd-phone"><?php esc_html_e('Phone number', 'wpstorm-notify') ?></label>
            <span class="edd-description">
				<?php esc_html_e('We use this to send order information.', 'wpstorm-notify') ?>
			</span>
            <input class="edd-input" type="text" name="edd_phone" id="edd-phone" placeholder="<?php esc_attr_e('Phone number', 'wpstorm-notify') ?>" />
        </p>
        <?php
    }
     ...
     */
    public function validate_mobile_field($valid_data, $data)
    {
        if (empty($data['edd_phone']) or !preg_match('/^09[0-9]{9}$/', $data['edd_phone'])) {
            edd_set_error('empty_phone', __('Enter a valid phone number.', 'wpstorm-notify'));
        }
    }
...
Wpstorm_Notify_Edd::get_instance();
