...
class Wpstorm_Notify_Comments
{
    ...
    {
        $comments_options = json_decode(get_option('wpstorm-notify_comments_options'), true);
        if ($comments_options) {
        ...
        add_filter('manage_edit-comments_columns', [$this, 'comments_table_columns']);
        add_action('manage_comments_custom_column', [$this, 'comments_table_columns_content'], 10, 2);
     ...
     * @since 1.0.0
     */
    public function comments_table_columns($my_cols)
    {
        $temp_columns = [
            'mobile' => __('Mobile', 'wpstorm-notify'),
        ];

        return array_slice($my_cols, 0, 3, true) + $temp_columns + array_slice($my_cols, 3, null, true);
    }

    public function comments_table_columns_content($column, $comment_ID)
    {
        switch ($column):
    ...
    public function add_mobile_field_to_comment_form()
    {
        ...
        $mobile_filed = '<p class="comment-form-phone uk-margin-top"><label for="mobile">' . esc_html__('Mobile', 'wpstorm-notify');
        if (self::$required_mobile_field === true) {
            $mobile_filed .= ' <span class="required">*</span></label>';
            $required     = 'required="required"';
        }
        $mobile_filed .= '<input class="uk-input uk-width-large uk-display-block" type="text" name="mobile" id="mobile" placeholder=" ' . esc_attr__('Like: 09*********', 'wpstorm-notify') . '" /></p>';
    ...
    public function save_mobile_field($comment_id)
    {
        if (isset($_POST['mobile'])) {
            $mobile = Wpstorm_Notify_Base::validate_mobile_number(sanitize_text_field($_POST['mobile']));
    ...
    public function verify_comment_input($comment_data)
    {
        if (empty($comment_data['comment_parent']) && self::$required_mobile_field === true && empty(sanitize_text_field($_POST['mobile']))) {
            wp_die(esc_html__('Error: Mobile number is required.', 'wpstorm-notify'));
        }
    ...
    public function send_admin_sms_on_new_comment(int $comment_ID)
    {
        ...
        if ($comment_post_type === 'post') {
            $data                  = self::comments_wpstorm_notify_shortcode($comment);
            ...
            $admin_number = Wpstorm_Notify_Base::$admin_number;
    ...
    public function send_author_sms_on_comment_approved($comment)
    {
        ...
        if (!empty($comment_parent_author_mobile) && !empty(self::$comment_reply_pattern)) {
            $reply_data = self::comments_wpstorm_notify_shortcode($comment);
    ...
    public function send_comment_sms($phone, $pattern, $data)
    {
        ...
        return Wpstorm_Notify_Ippanel::send_pattern($pattern, $phone, $input_data);
    }
    ...
    /**
     * Comments wpstorm-notify shortcode.
     */
    public function comments_wpstorm_notify_shortcode($comment)
    {
...
Wpstorm_Notify_Comments::get_instance();
