...
class Wpstorm_Notify_Login_Notify
{
    /**
     * Instance
     *
    ...
    {
        $login_notify_options = json_decode(get_option('wpstorm_notify_login_notify_options'), true);
        ...
        add_action('wp_login', [$this, 'admin_login_action'], 98, 2);
        add_action('wp_login', [$this, 'admin_roles_login_action'], 99, 2);
     ...
     * Admin login action.
     */
    public function admin_login_action($user_login, $user)
    {
        if (!in_array('administrator', (array) $user->roles)) {
            return;
        }
        $last_notification = get_user_meta($user->ID, 'wpstorm_notify_low_credit_notify_sent_timestamp', true);
        ...
        if ((int) $credit < 10000) {
            Wpstorm_Notify_Ippanel::send_admin_low_credit_to_admin();
            update_user_meta($user->ID, 'wpstorm_notify_low_credit_notify_sent_timestamp', time());
        }
    }
     ...
     */
    public function admin_roles_login_action($user_login, $user)
    {
    ...
    public function send_admins_login_notification_to_superadmin($pattern, $data)
    {
        ...
        return Wpstorm_Notify_Ippanel::send_pattern($pattern, Wpstorm_Notify_Base::$admin_number, $input_data);
    }
    ...
    {
        $selected_meta_key = Wpstorm_Notify_Base::$custom_phone_meta_keys_id;
        if ($meta_key !== $selected_meta_key) {
            return $check;
        }
        $phone = Wpstorm_Notify_Base::validate_mobile_number($meta_value);
        if (!$phone) {
            return $check;
        }
        ...
        if (!empty(Wpstorm_Notify_Base::$custom_phonebook_id)) {
            $list[0] = (object) [
                'number'       => $phone,
                'name'         => $user_info->display_name ?? '',
                'phonebook_id' => (int) Wpstorm_Notify_Base::$custom_phonebook_id
            ];
        ...
        Wpstorm_Notify_Base::send_welcome_message($phone, $object_id);

        return $check;
    }
}

Wpstorm_Notify_Login_Notify::get_instance();
