...
function Newsletter() {
  ...
  const originalState = {
    inputs: {
      news_phonebook: {
        ...
        label: __("Select phone book for newsletter", "wpstorm-notify"),
        options: [],
        noOptionsMessage: __("No options is available", "wpstorm-notify"),
        infoTitle: __("Shortcode:", "wpstorm-notify"),
        infoBody: __(
          "Use [wpstorm-notify-newsletter] as shortcode to display the newsletter form in your site.",
          "wpstorm-notify"
        ),
      },
      ...
      news_send_verify_pattern: {
        ...
        infoBody: __("%name% and confirmation code: %code%", "wpstorm-notify"),
      ...
      news_welcome: {
        ...
        type: "checkbox",
        label: __(
          "Welcome SMS to subscriber of the newsletter?",
          "wpstorm-notify"
        ),
      },
      news_welcome_pattern: {
        ...
        type: "text",
        label: __("Welcome SMS pattern code", "wpstorm-notify"),
        infoTitle: __("Usable variables:", "wpstorm-notify"),
        infoBody: __("%name%", "wpstorm-notify"),
        isDependencyUsed: false,
      },
      news_post_notify: {
        value: "",
        ...
        type: "checkbox",
        label: __("Send new posts to newsletter members?", "wpstorm-notify"),
      },
      news_post_notify_msg: {
        ...
        type: "textarea",
        label: __("Message content for new post", "wpstorm-notify"),
        infoTitle: __("Usable variables:", "wpstorm-notify"),
        infoBody: __(
      ...
      news_product_notify: {
        ...
        type: "checkbox",
        label: __("Send new product to newsletter members?", "wpstorm-notify"),
      },
      news_product_notify_msg: {
        ...
        type: "textarea",
        label: __("Message content for new product", "wpstorm-notify"),
        infoTitle: __("Usable variables:", "wpstorm-notify"),
        infoBody: __(
    ...
    isSaving: false,
    sendCount: 0,
    sectionName: __("Newsletter", "wpstorm-notify"),
  };
   ...
   * @since 1.0.0
   */
  const endpoint = "/wpstorm-notify/v1/newsletter_options";
  ...
  async function fetchSubscribers() {
    try {
      const getSubscribers = await AxiosWp.get(
        "/wpstorm-notify/v1/get_subscribers_from_db"
      );
  ...
  const handleSyncSubscribers = async () => {
    try {
      ...
      appDispatch({
        ...
        value: {
          message: __(
            "Congrats. Subscribers synced successfully.",
            "wpstorm-notify"
          ),
        },
      });
  ...
  const deleteSubscriber = async (subscriber_id) => {
    try {
      await AxiosWp.post("/wpstorm-notify/v1/delete_subscriber_from_db", {
  ...
  const deleteSubscribers = async (subscribers_ids) => {
    try {
      const res = await AxiosWp.post(
        "/wpstorm-notify/v1/delete_subscribers_from_db",
        {
  ...
  const deleteOneSubscriber = async (subscriber) => {
    const isConfirmed = await confirm(
      __("Do you want to delete that subscriber?", "wpstorm-notify")
    );
    if (isConfirmed) {
      ...
      appDispatch({
        type: "flashMessage",
        value: {
          message: __("subscriber deleted successfully.", "wpstorm-notify"),
        },
      });
    } else {
      appDispatch({
        type: "flashMessage",
        value: {
          message: __("Canceled. The subscriber still there.", "wpstorm-notify"),
          type: "error",
        },
      });
    }
  ...
  const handleDeleteSelectedSubscribers = async () => {
    const isConfirmed = await confirm(
      __("Do you want to delete the selected subscribers?", "wpstorm-notify")
    );
    ...
    if (isConfirmed) {
      ...
      appDispatch({
        type: "flashMessage",
        value: {
          message: __("Canceled. Subscribers still there.", "wpstorm-notify"),
          type: "error",
        },
      });
    }
  ...
  return (
    <div className="wpstorm-notify-newsletter-component">
      ...
      {state.newsletterSubscribers && (
        <div className="list-contacts">
          <table className="contact-list">
            <thead>
              <tr>
                ...
                <th>{__("Name", "wpstorm-notify")}</th>
                <th>{__("Phone Number", "wpstorm-notify")}</th>
          ...
          <div className="contact-list-actions">
            ...
            >
              {__("Delete Selected Subscribers", "wpstorm-notify")}
            </button>
            <button className="contact-sync" onClick={handleSyncSubscribers}>
              {__("Sync Subscribers", "wpstorm-notify")}
            </button>
          </div>
        </div>
      )}
