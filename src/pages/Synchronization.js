...
function Synchronization(props) {
  ...
  const originalState = {
    ...
    isSaving: false,
    sendCount: 0,
    sectionName: __("Synchronization", "wpstorm-notify"),
  };
  ...
  function syncWoo(e) {
    ...
    async function syncWoo() {
      let res;
      try {
        res = await AxiosWp.post("/wpstorm-notify/v1/sync_woo");
        console.log(res);
        if (res.data === true) {
          appDispatch({
            type: "flashMessage",
  ...
  function syncDigits(e) {
    ...
    async function syncDigits() {
      let res;
      try {
        res = await AxiosWp.post("/wpstorm-notify/v1/sync_digits");
        if (res.data === true) {
          appDispatch({
            type: "flashMessage",
            value: {
  ...
  function syncBookly(e) {
    ...
    async function syncBookly() {
      let res;
      try {
        res = await AxiosWp.post("/wpstorm-notify/v1/sync_bookly");
        console.log(res);
        if (res.data === true) {
          appDispatch({
            type: "flashMessage",
    ...
    <>
        ...
        <div className="card bg-light mb-3">
          <div className="card-body">
            <h5 className="card-title">
              {__(
                "Synchronization bookley users with phonebook",
                "wpstorm-notify"
              )}
            </h5>
            <a className="btn btn-info" onClick={syncBookly}>
              {__("Bookley synchronization", "wpstorm-notify")}
            </a>
          </div>
        </div>
      ) : (
        ...
        <div className="card bg-light mb-3">
          <div className="card-body">
            ...
            <button className="btn btn-info" onClick={syncWoo}>
              {__("Woocommerce synchronization", "wpstorm-notify")}
            </button>
          </div>
        </div>
      ) : (
      ...
      {props.integratedPlugins.digits.use ? (
        <div className="card bg-light mb-3">
          <div className="card-body">
            <h5 className="card-title">
              {__("Synchronization digits users with phonebook", "wpstorm-notify")}
            </h5>
            <a className="btn btn-info" onClick={syncDigits}>
              {__("Digits synchronization", "wpstorm-notify")}
            </a>
          </div>
        </div>
      ) : (
