...
function Support(props) {
  ...
  const originalState = {
    inputs: {
      subject: {
        ...
        name: "subject",
        type: "text",
        label: __("Subject:", "wpstorm-notify"),
      },
      message: {
        value: "",
        hasErrors: false,
    ...
    sectionName: __("Report Issues", "wpstorm-notify"),
  };
  ...
  const handleSubmit = async (e) => {
    ...
    async function reportIssues() {
      try {
        await AxiosWp.post("/wpstorm-notify/v1/send_feedback_message", {
    ...
    <div>
      ...
      <div>
        <SettingsForm
          ...
          buttonText={__("Report Issues", "wpstorm-notify")}
        />
      </div>
    </div>
  );
